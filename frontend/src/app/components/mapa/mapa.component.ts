...
@Component({
  selector: 'app-mapa',
  template: '<div id="map" style="height: 550px; width: 100%; border-radius: 12px;"></div>'
})
export class MapaComponent implements AfterViewInit, OnChanges {
  @Input() pais: string = 'Peru';
  ...
  private initMap(): void {
    const center = this.getCountryCenter();
    const zoom = this.getCountryZoom();
  ...
  private getCountryCenter(): L.LatLngExpression {
    ...
    return centers[this.pais] || [-9.19, -75.01];
  }
  ...
  private getCoordinates(nombre: string, pais: string): L.LatLngExpression | null {
    ...
    const colombiaCoords: { [key: string]: L.LatLngExpression } = {
      ...
      'Bolívar': [8.67, -74.10], 'Boyacá': [5.53, -73.36],
      'Caldas': [5.07, -75.52], 'Caquetá': [1.61, -75.60],
      'Casanare': [5.34, -72.41], 'Cauca': [2.44, -76.61],
      'Cesar': [9.33, -73.50], 'Chocó': [5.69, -76.65],
      'Córdoba': [8.75, -75.88], 'Cundinamarca': [4.60, -74.10],
      'Guainía': [3.86, -67.92], 'Guaviare': [2.57, -72.64],
      'Huila': [2.92, -75.29], 'La Guajira': [11.55, -72.91],
      'Magdalena': [10.43, -74.41], 'Meta': [3.28, -73.08],
      'Nariño': [1.21, -77.27], 'Norte de Santander': [7.90, -72.50],
      'Putumayo': [1.15, -76.65], 'Quindío': [4.54, -75.67],
      'Risaralda': [5.34, -75.70], 'San Andrés': [12.58, -81.70],
      'Santander': [6.64, -73.65], 'Sucre': [9.30, -75.40],
      'Tolima': [4.09, -75.20], 'Valle del Cauca': [3.45, -76.53],
      'Vaupés': [0.85, -70.80], 'Vichada': [6.17, -69.50]
    };
    ...
    const uruguayCoords: { [key: string]: L.LatLngExpression } = {
      ...
      'Florida': [-33.85, -56.07], 'Lavalleja': [-33.92, -54.96],
      'Maldonado': [-34.90, -54.95], 'Montevideo': [-34.90, -56.19],
      'Paysandú': [-32.32, -58.08], 'Río Negro': [-32.78, -57.45],
      'Rivera': [-30.90, -55.55], 'Rocha': [-34.48, -54.33],
      'Salto': [-31.38, -57.96], 'San José': [-34.33, -56.70],
      'Soriano': [-33.40, -57.45], 'Tacuarembó': [-31.73, -55.98],
      'Treinta y Tres': [-33.23, -54.38]
    };
    
    // Coordenadas de México (actualizadas)
    ...
    // Seleccionar el objeto de coordenadas según el país
  ...
  private getColorByHours(horas: number): string {
    if (horas <= 35) return '#FFF5F5';      // Muy bajo
    if (horas <= 40) return '#FFD4D4';      // Bajo
    if (horas <= 45) return '#FFA8A8';      // Medio bajo
    if (horas <= 50) return '#FF8A8A';      // Medio
    if (horas <= 55) return '#FF5252';      // Medio alto
    if (horas <= 60) return '#FF0000';      // Alto
    return '#8B0000';                        // Muy alto
 }
}